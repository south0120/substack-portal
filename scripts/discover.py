...
# certifi が無い環境(ローカルのpython3.14等)でもSSLで落ちないように。公開Substackの取得のみ。
try:
...
def http_get(url, timeout=20, as_json=False):
    ...
    for attempt in range(5):
        try:
            raw = urllib.request.urlopen(urllib.request.Request(target, headers=headers), timeout=timeout, context=SSL_CTX).read()
